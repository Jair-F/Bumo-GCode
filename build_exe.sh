...
# python -m nuitka --windows-console-mode=attach --jobs=16 --standalone --onefile --mingw64 --lto=no --assume-yes-for-downloads --include-data-dir=data=data --onefile-windows-splash-screen-image=data/splash.png --windows-icon-from-ico=data/icon.ico --company-name="Koeger" --product-name="Bumo-GCode" --file-version=0.0.0 --product-version=0.0.0 --file-description="Bumo-GCode Copier" --copyright="2026" --output-filename=app.exe src/app.py
